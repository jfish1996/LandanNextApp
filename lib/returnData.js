...
export const returnNavData = () => {
  ...
  let { data, fetching, error } = results;
  if (fetching) return { fetching };
  if (error) return <p>error {error}</p>;
  results = data.categories.data;
  let subSections = data?.attributes?.sections?.data;
...
export const returnShoppingStatus = () => {
  const [results] = useQuery({
    query: SHOPPING_MESSAGE_QUERY,
  });
  const { data, fetching, error } = results;
  if (fetching) return { fetching };
  if (error) return <p>error {error}</p>;
  const success = data.shoppingStatusMessages.data[0];
  const failed = data.shoppingStatusMessages.data[1];
